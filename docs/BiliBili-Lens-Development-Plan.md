...
syrds `/get_replies` 查询接口当前限制：

```text
使用 nginx 原生 limit_req
每 IP 每 4 秒恢复 1 次额度
桶容量上限 15 次
如果瞬时并发 16 次，第 16 次会直接返回 429
```

项目不使用 `/ai_summary`，只需要对 `/get_replies` 做限流。

实现要求：

- 后端本地实现 token bucket：容量 15，每 4 秒恢复 1 个 token。
- 批量采集任务必须通过统一调度器调用 syrds client。
- 不允许在多个模块里绕过 syrds client 直接请求 `/get_replies`。
...
- 使用 token bucket / Redis + BullMQ 适配 syrds 每 IP 每 4 秒恢复 1 次额度、桶容量 15 的限流约束。
